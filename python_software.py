...
class BitcoinAnalyzer:
    ...
    def collect_data(self, start_date='2020-01-01'):
        """Processa dados históricos de preços e volumes da Bitcoin."""
        self.data = yf.download(self.symbol, start=start_date)
        print("Dados coletados com sucesso.")

    def calculate_indicators(self):
        """Aplica indicadores técnicos: MA, RSI, Bandas de Bollinger e MACD."""
    ...
    def plot_data(self):
        """Gera visualizações gráficas para identificação de tendências e padrões de preços."""
        if self.data is None:
            raise Exception("Dados não carregados. Execute collect_data primeiro.")

        plt.figure(figsize=(14, 10))

        # Gráfico dos preços e Médias Móveis
        plt.subplot(3, 1, 1)
